  ...
  return (
    <div class="shell">
      <header class="topbar">
        ...
        {(filter === "planning" || filter === "today") && <div class="planning-topbar-tools" role="toolbar" aria-label="Planning controls"><button class={`compact-control icon-only-control ${searchOpen || search ? "active" : ""}`} aria-label="Search tasks" title="Search tasks" aria-controls="planning-search" aria-expanded={searchOpen} aria-pressed={searchOpen} onClick={() => { setSearchOpen(!searchOpen); if (!searchOpen) window.setTimeout(() => searchInput.current?.focus(), 0); }}><Icon name="search" /></button><details class="view-menu" open={viewOpen} onToggle={(event) => setViewOpen(event.currentTarget.open)}><summary class="compact-control icon-only-control" aria-label="View planning options" title="View planning options" aria-expanded={viewOpen}><Icon name="view" />{planningActiveStates.length > 0 && <span class="filter-count" aria-label={`${planningActiveStates.length} active view filters`}>{planningActiveStates.length}</span>}</summary><div class="view-popover"><p class="view-context">Week starts {Number(settings.firstDayOfWeek ?? 1) === 0 ? "Sunday" : Number(settings.firstDayOfWeek ?? 1) === 6 ? "Saturday" : "Monday"} · Day starts {String(settings.startTimeOfDay ?? "00:00")}</p><button aria-pressed={filter === "today"} onClick={(event) => applyDisclosureAction(() => setFilter(filter === "today" ? "planning" : "today"), event.currentTarget)}>{filter === "today" ? "Show all dates" : "Today only"}</button><label class="month-control">Calendar month<input type="month" value={planningMonth} onInput={(event) => applyDisclosureAction(() => { setPlanningMonth(event.currentTarget.value); setFilter("planning"); }, event.currentTarget)} /></label><button aria-pressed={showCompleted} onClick={(event) => applyDisclosureAction(() => setShowCompleted(!showCompleted), event.currentTarget)}>{showCompleted ? "Hide completed" : "Include completed"}</button><button aria-pressed={reorderEnabled} title={planningReorderHelp} onClick={(event) => applyDisclosureAction(() => { setFilter("planning"); setReorderEnabled(!reorderEnabled); }, event.currentTarget)}>{reorderEnabled ? "Finish ordering" : "Reorder tasks"}</button></div></details><button class="compact-control add-context" aria-keyshortcuts="A" onClick={openGlobalAdd}><Icon name="plus" />Add task</button></div>}
      ...
      <main class="workspace">
        {(filter === "planning" || filter === "today") ? <><h1 class="sr-only">{filter === "today" ? "Planning · Today" : "Planning"}</h1>{planningActiveStates.length > 0 && <div class="planning-active-filters" aria-label="Active planning filters">{planningActiveStates.map((state) => <span class="active-filter-badge" key={state}>{state}</span>)}</div>}</> : <div class="list-header"><div class="list-heading"><h1>{filter === "reports" ? "Report" : `${filter[0]?.toUpperCase()}${filter.slice(1)}`}</h1></div><div class="header-actions">{!(["reports", "trash"] as TaskView[]).includes(filter) && <button class="compact-control add-context" aria-keyshortcuts="A" onClick={openGlobalAdd}><Icon name="plus" />Add task</button>}</div></div>}
        {(filter === "planning" || filter === "today") && searchOpen && <div class="planning-search" id="planning-search"><input ref={searchInput} type="search" value={search} placeholder="Search tasks, notes, and tags" aria-label="Search tasks" onInput={(event) => setSearch(event.currentTarget.value)} /><button class="icon-button" aria-label="Close search" onClick={() => { setSearchOpen(false); setSearch(""); }}><Icon name="close" /></button></div>}
